...
parser.add_argument("--pretrained_dispnet", default='/remote-home/yxfei/depth/SwinDepth/checkpoints/ht_dcmnet/outputs/230506/152929/models/weights_16/', type=str, help="pretrained DispNet path")
...
parser.add_argument("--dataset-dir", default='/remote-home/share/KITTI/KITTI_fengyi/', type=str, help="Dataset directory")
parser.add_argument("--dataset-list", default='/remote-home/yxfei/depth/SwinDepth/test_files_eigen.txt', type=str, help="Dataset list file")
parser.add_argument("--output-dir", default='/remote-home/yxfei/depth/SwinDepth/checkpoints/predictions', type=str, help="Output directory for saving predictions in a big 3D numpy file")
...
def main():
    ...
    if args.gt_type == 'KITTI':
        from depth_evaluation_utils import test_framework_KITTI as test_framework

    num_layers = 18
    encoder = networks.ResnetEncoder(num_layers, "pretrained")
    encoder_path = os.path.join(args.pretrained_dispnet, "encoder.pth")
    ...
    decoder_path = os.path.join(args.pretrained_dispnet, "depth.pth")
    ...
    for j, sample in enumerate(tqdm(framework)):
        ...
        # from ipdb import set_trace
        # set_trace()
        pred_disp = depth_decod(encoder(tgt_img))['disp',0]
    ...
    if args.output_dir is not None:
        np.save(output_dir/'predictions_0503.npy', predictions)
